...
export class WechatComponent {
  ...
  constructor(
    ...
    // 默认使用内存存储（开发环境）
    this.getToken = options.getToken || (async () => this.cachedToken);
    ...
    this.saveToken =
      ...
      (async (token) => {
        ...
        if (process.env.NODE_ENV === 'production') {
          console.warn("[wechat-component] Don't save component token in memory in production!");
        }
      });
  }
   ...
   */
  private async withAccessToken<T>(fn: (accessToken: string) => Promise<T>): Promise<T> {
   ...
   */
  async getAuthorizeURL(redirectUri: string, preAuthCode?: string): Promise<string> {
   ...
   * @param authorizerAppid - 授权方 appid
   */
  async getAuthorizerInfo(authorizerAppid: string): Promise<GetAuthorizerInfoResult> {
    return this.withAccessToken(async (accessToken) => {
   ...
   */
  async getOAuthAccessToken(authorizerAppid: string, code: string): Promise<OAuthAccessToken> {
    return this.withAccessToken(async (componentAccessToken) => {
      ...
      return this.request<OAuthAccessToken>(`${OAUTH_PREFIX}access_token?${params}`);
    });
  }
  ...
  async refreshOAuthAccessToken(
    ...
    return this.withAccessToken(async (componentAccessToken) => {
      ...
      return this.request<OAuthAccessToken>(`${OAUTH_PREFIX}refresh_token?${params}`);
    });
  }
}
